...
- And a seventh move for the case every other one assumes away — a quit that came apart. The zero era used to be a one-way door: its undos expire the day they're offered, and every other move needs a capped rung still ahead of you, so a return had no path back at all. **Start again** closes the first quit's zero era as an interlude, re-enters the ladder at the ceiling the recent log shows, and walks a fresh descent in half-steps with a refilled budget. Nothing is erased — the whole first descent and the clean run it reached stay exactly as logged, and the new ladder starts lower *because* of them. Reachable from the schedule card the whole time a quit is running; leads the Today screen only once the log shows a pattern rather than one night
